...
export function registerQueryData(server: McpServer, env?: QueryEnv): void {
	...
	server.registerTool(
		"cdc_query_data",
		{
			title: "Query Staged CDC Data",
			description:
				"Query staged CDC Open Data results using SQL. Use this when responses are too large and have been staged with a data_access_id.",
			inputSchema: {
